...
// 将所有status数组中值为1的索引对应位置的v数组中的元素调整到前0～(j-1)的位置，此时数组大小设置为j
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        if (n_max_cnt > 0)
        {
            ...
            /**
             * 提取新的角点
             * MAX_CNT - forw_pts.size()为提取的最大个数
             * 新提取的角点坐标保存在n_pts中
